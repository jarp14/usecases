 ...
 * @generated
 */
public class SystemItemSemanticEditPolicy extends UsecasesBaseItemSemanticEditPolicy {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (UsecasesVisualIDRegistry.getVisualID(node)) {
			case SystemSystemUseCasesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (UsecasesVisualIDRegistry.getVisualID(cnode)) {
